...
<script>	
  ...
  export let buffer = 5;
	...
	export let colors = color ? [color] : ['#3C388E', '#DF0667', '#206095', '#A8BD3A', '#003C57', '#27A0CC', '#118C7B', '#F66068', '#746CB1', '#22D0B6', 'lightgrey'];
	export let r = 4;
...
<div class="chart-container" style="height: {typeof height == 'number' ? height + 'px' : height }">
    ...
    <slot name="back"/>
      ...
      {/if}
			<Scatter {selected} {hovered} {coords} {placeLabel} {highlighted} {overlayFill}/>
			{#if select || hover}
				<Voronoi {select} bind:selected {hover} bind:hovered {highlighted} on:hover={doHoverScatter} on:select/>
			{/if}
			{#if labels}
				<Labels {hovered} {coords} {placeLabel} {selected}/>
			{/if}
			<slot name="svg"/>
		</Svg>
	  <slot name="front"/>
