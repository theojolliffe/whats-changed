<script>
	...
	let selectedItem = selectedb ? options.find(d => { d[value] == selectedb[value] }) : null;
	let expanded = false;
	let filter = '';
	let active = null;
	let filtered;
	...
	$: if (selectedPrev != selectedb) {
		selectedItem = options.find(d => d[value] == selectedb[value]);
		selectedPrev = selectedb;
	}
</script>
