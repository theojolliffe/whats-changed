...
'W92000004']
	...
	function loadSubTopic(code) {
		if (code=="all") {
			...
			let subplacesHigh = subplacesOb.sort(function(a, b) { return b['story'][0]['value'] - a['story'][0]['value'] }).slice(0,1)
	...
	function loadTopic(code) {
		...
		let placesHigh = placesOb.sort(function(a, b) { return b['story'][0]['value'] - a['story'][0]['value'] }).slice(0,1)
